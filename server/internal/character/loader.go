...
func LoadFromFile(path string) (*Store, error) {
	...
	store := &Store{byID: make(map[string]Character), list: make([]Character, 0, len(f.Characters))}
	for _, c := range f.Characters {
		store.byID[c.ID] = c
		store.list = append(store.list, Character{
...
func (s *Store) List() []Character { return s.list }
